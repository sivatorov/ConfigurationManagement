...
![Версия](https://img.shields.io/badge/Версия-0.3.9.49-1F6FEB) ![.NET](https://img.shields.io/badge/.NET-10-512BD4) ![Windows/WPF](https://img.shields.io/badge/Windows-WPF-4B8BBE) ![Linux/Avalonia](https://img.shields.io/badge/Linux-Avalonia%2011-8B5CF6) ![Лицензия](https://img.shields.io/badge/Лицензия-Open%20Source-success)
...
- **Темы оформления**: светлая/тёмная, собственные цветовые схемы с двумя палитрами и живым предпросмотром; автогенерация пары светлой и тёмной палитр из одного базового цвета («Создать схему из цвета…»).
...
- **Задания по расписанию (меню «Утилиты»)** — резервная копия, обновление конфигурации ИБ из `.cf` (`/LoadCfg` + `/UpdateDBCfg`), связка «Копия → обновление» и обновление самого приложения по времени и дням недели; выполняются фоновым планировщиком, пока приложение запущено.
...
- **Временная блокировка приложения паролем** — защита интерфейса на время отсутствия пользователя (пароль в виде PBKDF2-хэша).
...
- **Консоль администрирования серверов 1С (CTRL+ALT+S)** — запуск консоли администрирования серверов для клиент-серверных баз (оснастка `1CV8Servers.msc` / `rac`).
...
| **Клиент-серверная** (Windows) | Реальное внешнее подключение к базе через COM-коннектор 1С (процесс-агент `ComReadHost`, `ReadConfigurationInfo`) с таймаутом **до 8 секунд на базу** |
...
> большое число баз (очередь из таймаутов по 8 секунд). Список баз можно проверить повторно —
