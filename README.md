...
- результат каждой базы публикуется **сразу по мере готовности**: значок меняется на тип базы
  (янтарная — файловая, синяя — веб, фиолетовая — клиент-серверная) либо на красный крестик «недоступна»;
