...
namespace Configuration_Management
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private Control BuildMainArea()
        {
            ...
            // Автопрокрутка к выбранному элементу отключена: штатная реализация
            // прокручивает список к ПЕРВОМУ контейнеру с данными выбранной базы,
            // а закреплённая база есть в дереве дважды («Закреплённые» и своя группа) —
            // после клика или пересборки список «уезжал» бы к копии в начале списка
            // (issue #301). Прокрутку ведут свои методы: BringRowIntoView (клавиши),
            // SelectAndReveal («Найти в списке») и RestoreTreeSelection (пересборка).
            _tree.AutoScrollToSelectedItem = false;
